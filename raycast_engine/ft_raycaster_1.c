...
/*   Updated: 2025/09/16 17:17:50 by nsamarin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	x->ref.x = (int)(t->wall_x * x->size);
	if ((t->side == 0 && t->ray_dir_x < 0) || (t->side == 1 && t->ray_dir_y > 0))
...
{
	x->step = 1.0 * x->size / t->line_height;
	x->pos = (t->draw_start - d->mapy / 2 + t->line_height / 2) * x->step;
}
...
void	ft_ray_cast(t_play *p, t_data *d)
{
	...
	while (a < d->mapx)
	{
		ft_track_init(&t);
		t.cam_x = 2 * a / 640 - 1;
		t.ray_dir_x = p->dir_x + p->plane_x * t.cam_x;
		t.ray_dir_y = p->dir_y + p->plane_y * t.cam_x;
		t.map_x = (int)p->pos.x;
		t.map_y = (int)p->pos.y;
