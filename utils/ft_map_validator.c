...
/*   Updated: 2025/08/13 12:07:49 by jacky599r        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Helper to process a single line within ft_check_map_config
static int ft_process_config_line(t_data *data, char *trimmed_line, int *map_start_index, int *param_count)
{
    if (ft_is_empty_line(trimmed_line))
    {
        if (*map_start_index != -1 && !data->map.eom)
            data->map.eom = 1;
        return (PASS); // Indicates line was handled, no error
    }
    if (*map_start_index == -1 && ft_is_valid_param_identifier(trimmed_line))
    {
        ...
        *map_start_index = -1; // Indicate that map section has started, actual index handled in main loop
...
int ft_identify_map_properties(t_data *data, int start_index)
{
    ...
    data->map.high = 0;
    data->map.wide = 0;
    while (data->map.og_map[current_line_index] && !ft_is_empty_line(ft_strtrim(data->map.og_map[current_line_index], " \n\t\v\f\r")))
    {
        ...
        current_width = ft_strlen(ft_strtrim(line, " \n\t\v\f\r"));
